...
\msg_new:nnn  { l3-weighted-stack } { generic typeout }
  { Message:~~ #1 }
...
\cs_new:Nn \ws_push:Nnn
  {
    \msg_info:nnn { l3-weighted-stack } { generic typeout }
      { In~ push. }
    \seq_get:cNTF
      ...
      \g_tmpa_int % put the weight into this temp variable
      {
        ...
        \bool_until_do:nn
          {
            \seq_if_empty_p:c { \__ws_get_values:N #1 }
            ||
...
%            \seq_gpop:cN { \__ws_get_values:N  #1 } \l_tmpa_tl
            
            % Pop the weight stack and throw it away
%            \seq_gpop:cN { \__ws_get_weights:N #1 } \l_tmpa_int
...
\cs_new:Nn \__ws_gpop:NNN
  {
    % Pop the value stack and throw it away
    \seq_gpop:cN { \__ws_get_values:N  #1 } #2
    
    % Pop the weight stack and throw it away
    \seq_gpop:cN { \__ws_get_weights:N #1 } #3
  }
