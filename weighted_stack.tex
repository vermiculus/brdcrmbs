...
\cs_new:Nn \__ws_get_values:N
  {
    \use:c { \cs_to_str:N #1 _values_seq }
  }

\cs_new:Nn \__ws_get_weights:N
  {
    \use:c { \cs_to_str:N #1 _weights_seq }
  }

\cs_new:Nn \ws_new:N
  {
    \seq_new:N { \__ws_get_values:N  #1 }
    \seq_new:N { \__ws_get_weights:N #1 }
  }
      ...
      \g_tmpa_int % put the weight into this temp variable
      {
        ...
        \bool_until_do:nn
          ...
          }
          {
            ...
            \seq_gpop:NN { \__ws_get_values:N  #1 } \l_tmpa_tl

            % Pop the weight stack and throw it away
            \seq_gpop:NN { \__ws_get_weights:N #1 } \l_tmpa_int

            % Update the current weight on the stack
            \seq_get:NN { \__ws_get_weights:N #1 } \l_tmpa_int
